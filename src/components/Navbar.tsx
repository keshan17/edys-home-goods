        ...
        <ul className="hidden md:flex items-center gap-8">
          {navLinks.slice(0, 3).map((link) => (
            <li key={link.label}>
              <a
                href={link.href}
                className="text-primary-foreground text-xs tracking-[0.2em] font-body hover:opacity-70 transition-opacity"
              >
                {link.label}
              </a>
            </li>
          ...
          <div className="container py-6 flex flex-col gap-4">
            {navLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="text-primary-foreground text-sm tracking-[0.15em] font-body py-2 hover:opacity-70 transition-opacity"
